        ...
        <div className="flex justify-between items-center pt-6 pb-4">
          ...
          <div className="flex items-center space-x-4">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="bg-blue-500 text-white hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:ring focus:ring-blue-300 transition-colors duration-150 rounded-full px-4 py-2 text-sm font-medium"
                >
                  切换 Sprint
                </Button>
              </DropdownMenuTrigger>
            ...
            <Button
              variant="outline"
              size="sm"
              className="bg-gray-200 text-gray-700 hover:bg-gray-300 active:bg-gray-400 focus:outline-none focus:ring focus:ring-gray-300 transition-colors duration-150 rounded-full px-4 py-2 text-sm font-medium"
        ...
        <main className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-10 gap-6 w-full py-4">
          ...
          <Card className="col-span-1 md:col-span-2 lg:col-span-6">
            <CardHeader>
              ...
              <CardDescription>
                View sprint and product burndown charts.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <LineChart
              ...
              <div className="grid grid-cols-2 gap-4 pt-10">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                  <div className="flex items-center justify-between">
                    <div className="text-lg font-bold">迭代速度</div>
                    <div className="text-gray-500 dark:text-gray-400">
                      <GaugeIcon className="w-5 h-5 inline-block" />{" "}
                      {
                        // 便利velocityData的count字段,计算平均值取整，舍弃0的数据
                        Math.round(
                          velocityData
                            .map((item) => item.count)
                            .filter((item) => item !== 0)
                            .reduce((a, b) => a + b, 0) /
                            velocityData.filter((item) => item.count !== 0)
                              .length
                        )
                      } h
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                  <div className="flex items-center justify-between">
                    <div className="text-lg font-bold">工作负载</div>
                    <div className="text-gray-500 dark:text-gray-400">
                      <ActivityIcon className="w-5 h-5 inline-block" />{" "}
                      {
                        // 便利workloadData的value字段,计算平均值取整，舍弃0的数据
                        Math.round(
                          workloadData
                            .map((item) => item.value)
                            .filter((item) => item !== 0)
                            .reduce((a, b) => a + b, 0) /
                            workloadData.filter((item) => item.value !== 0)
                              .length
                        )
                      } h
                    </div>
...
function SprintCard({ sprint, backlogList, taskList }) {
  ...
  return (
    <Card className="col-span-1 md:col-span-2 lg:col-span-4 bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-shadow duration-300">
      <CardHeader className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
        <div className="flex items-center justify-between">
          <CardTitle className="text-2xl font-bold text-gray-800 dark:text-gray-100">{sprint.name}</CardTitle>
      ...
      <div className="bg-gradient-to-r from-blue-100 to-green-100 dark:from-blue-900 dark:to-green-900 p-6 rounded-lg shadow-md">
        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-3">Sprint 目标</h3>
        <div className="text-gray-700 dark:text-gray-300 text-lg italic">&ldquo;{sprint.goal}&rdquo;</div>
      </div>

        <div className="grid grid-cols-2 gap-4">
          <div className="bg-blue-100 dark:bg-blue-900 p-4 rounded-lg shadow-inner text-center">
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-300">
              {daysLeft <= 0 ? "已结束" : `${daysLeft} 天`}
            </div>
            <div className="text-base font-medium text-blue-500 dark:text-blue-400 mt-1">剩余时间</div>
          </div>
          <div className="bg-green-100 dark:bg-green-900 p-4 rounded-lg shadow-inner text-center">
            <div className="text-3xl font-bold text-green-600 dark:text-green-300">
              {completionPercentage}%
            </div>
            <div className="text-base font-medium text-green-500 dark:text-green-400 mt-1">完成度</div>
          </div>
        </div>
 
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-gray-100 dark:bg-gray-700 p-6 rounded-lg shadow-inner">
            <div className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-3">每日站会</div>
            <div className="text-gray-600 dark:text-gray-300 text-lg">{sprint.dailyStandup}</div>
          </div>
          <div className="bg-gray-100 dark:bg-gray-700 p-6 rounded-lg shadow-inner">
            <div className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-3">Sprint 演示</div>
            <div className="text-gray-600 dark:text-gray-300 text-lg">{sprint.sprintReview}</div>
          </div>
        </div>
        <div className="bg-gray-100 dark:bg-gray-700 p-6 rounded-lg shadow-inner">
          <div className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-3">Sprint 待办事项（预估）</div>
          <ul className="list-disc pl-6 text-gray-600 dark:text-gray-300 space-y-2">
            {backlogList.map((backlog) => (
              <li key={backlog.id} className="text-lg">
